...
class FreshdeskError extends Error{
	constructor (message, data, res) {
		super()
		this.requestFormArrayFields = ["attachments", "cc_emails", "fwd_emails", "reply_cc_emails", "tags", "to_emails"];
...
function makeRequest(method, auth, url, qs, data, cb) {		// eslint-disable-line max-params
	...
	for (let i = 0; i < dataKeys.length; i++) {
		...
		if (this.requestFormArrayFields.includes(key) ) {
			for (let j = 0; j < data[key].length; j++) {
				form.append(`${key}[]`, data[key][j])
			}
			continue;
		}
