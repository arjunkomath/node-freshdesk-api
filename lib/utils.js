...
async function makeRequest(method, auth, url, qs, data, cb) {
	...
	try {
		...
		const data = response.statusCode !== 204 ? await response.body.json() : response.body.text()
		return createResponseHandler(cb)(null, response, data, req);
	} catch (error) {
		if (error.response) {
			const data = error.response.statusCode ? await error.response.body.json() : null
			...
			const data = error.request.body ? await error.request.body.json() : rror.request.body.text()
			return createResponseHandler(cb)(
