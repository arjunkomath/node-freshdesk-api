 ...
 * @param {string} apiKeyOrCredentials     API key or credentials object (for example `{username: 'test@test.com, password: 'mypass'}`)
 * @public
 */
const Freshdesk = function (baseUrl, apiKeyOrCredentials) {
	this.baseUrl = baseUrl;
	const username = apiKeyOrCredentials.username || apiKeyOrCredentials;
	const password = apiKeyOrCredentials.password || "X";

	this._auth = 'Basic ' + new Buffer(username + ":" + password).toString('base64');
}
...
Freshdesk.prototype = Object.create({
	...
	listAllGroups(cb) {
		makeRequest('GET', this._auth, `${this.baseUrl}/api/v2/groups`, null, null, cb)
	},
